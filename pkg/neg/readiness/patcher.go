...
func (p *patcher) syncPodNegReadiness(key string) error {
	...
	if !ok {
		klog.Warning("Pod %q could not be found in tracker", objKey.String())
	} else {
	...
	expectedCondition := v1.PodCondition{Type: NEGReadinessCondition}
	message := ""
	reason := ""
	if podReady {
		expectedCondition.Status = v1.ConditionTrue
		reason = "LoadBalancerNegReady"
		message = fmt.Sprintf("Pod is healthy in all associated Google Cloud Loadbalancer managed by ingress. Marking condition %q to True.", NEGReadinessCondition)
	} else {
...
func (p *patcher) handleErr(err error, key interface{}) {
	...
	if err != nil {
		klog.Warning("Failed to retrieve pod %q from store: %v", key, err)
	} else if !exists {
		klog.Warning("Pod %q no longer exists", key)
	} else {
