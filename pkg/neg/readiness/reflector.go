...
func NewReadienssReflector(ctx *context.ControllerContext) Reflector {
	...
	return &readinessReflector{
		...
		svcSynced:        ctx.ServiceInformer.HasSynced,
...
func (r *readinessReflector) synced() bool {
	return r.podSynced() && r.svcSynced()
}
...
func (r *readinessReflector) removeNeg(neg negName) {
	...
	if !ok {
		klog.Warning("NEG %q not found in tracker.", neg.String())
		return
	}
